...
### Given function $\color{DeepSkyBlue} {\ f(x)}$ = $\color{DeepSkyBlue}  {x^2 + 3x + 2}$, find the derivative of $\color{DeepSkyBlue} {\ f(x)}$ with respect to \color{blue} {( x )}, and evaluate it at $\color{blue} {( x = 1 )}$.
